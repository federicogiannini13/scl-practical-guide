...
def build_conf_df(root, dataset, df, experiences, conf, train="train"):
    ...
    df_conf.to_csv(os.path.join(root, "datasets", f"{dataset}_incremental_{conf}conf_{train}.csv"),
                   index=False)
    return df_conf
...
def make_exp_incremental_train(root, dataset, df, set1, set2, nconfs=10):
    try:
        with open(os.path.join(root, "datasets", f"{dataset}_incremental_confs.pkl"), "rb") as f:
            confs = pickle.load(f)
    except:
        confs = []
    count_confs = 0
    ...
    while (count_confs < nconfs):
        ...
        dfs.append(df_conf)
    with open(os.path.join(root, "datasets", f"{dataset}_incremental_confs.pkl"), "wb") as f:
        pickle.dump(confs, f)
    return dfs
...
def build_conf_df_sml(root, dataset, df, conf, conf_number, train="train"):
    n_split = 5
    dfs = [[] for _ in range(n_split)]
    for target in df["target"].unique():
        df_target = df[df["target"] == target].sample(frac=1)
        len_split = len(df_target) // n_split
        ranges = [(i * len_split, (i + 1) * len_split) for i in range(n_split - 1)] + [
    ...
    final_df.to_csv(os.path.join(root, "datasets", f"{dataset}_sml_{conf_number}conf_{train}.csv"),
                    index=False)
    return final_df


def make_exp_sml_train(root, dataset, df, nconfs=10):
    try:
        with open(os.path.join(root, "datasets", f"{dataset}_sml_confs.pkl"), "rb") as f:
            confs = pickle.load(f)
    except:
        confs = []
    count_conf = 0
    ...
    while count_conf < nconfs:
        ...
        confs.append(conf)
        count_conf += 1
        final_dfs.append(build_conf_df_sml(root, dataset, df, conf, len(confs), "train"))
    with open(os.path.join(root, "datasets", f"{dataset}_sml_confs.pkl"), "wb") as f:
        pickle.dump(confs, f)
    return final_dfs


def make_exp_sml_test(root, dataset, df):
    with open(os.path.join(root, "datasets", f"{dataset}_sml_confs.pkl"), "rb") as f:
        confs = pickle.load(f)
    dfs = []
    for count, experiences in enumerate(confs):
